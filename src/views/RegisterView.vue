...
import { getAuthErrorMessage, loginWithGoogle, registerUser } from '../services/authService'
...
const handleGoogleRegister = async () => {
  ...
  try {
    ...
    message.value = { type: 'success', text: 'Sesión iniciada con Google. Redirigiendo...' }

    setTimeout(() => {
      const redirectPath = String(route.query.redirect ?? '/mis-citas')
      router.push(redirectPath)
    }, 600)
  } catch (error) {
    console.error(error)
  ...
  <section class="auth-view app-container">
    <article class="panel-card auth-card">
      ...
      <form class="auth-form" @submit.prevent="handleSubmit" novalidate>
        ...
        <div class="divider"><span>o regístrate rápido</span></div>
      ...
      <p class="helper-text">
        ¿Ya tienes cuenta?
        <RouterLink :to="{ path: '/login', query: route.query.redirect ? { redirect: route.query.redirect } : {} }">
          Iniciar sesión
        </RouterLink>
      </p>
    </article>
